...
public class DepartmentSummaryActionBean extends CustomActionBean{
	...
	public Resolution defaultAction(){
		try{
			departments = scoreBean.getAllManageDeparmentsByUser(context.getUser());
...
//			cal2.add(Calendar.MONTH, 1);
			...
			if(selectDepartment == null)
				ds = scoreBean.getDepartmentScoresByDepartmetnIdORWorkerId(null,context.getUser().getEmployee());
			else
				ds = scoreBean.getDepartmentScoresByDepartmetnIdORWorkerId(selectDepartment,null);
...
//			summary = scoreBean.getDepartmentScoreSummary(context.getUser(),month,ds.getDepartment().getId());
			...
			departmentWeekEmployee = scoreBean.getScoreEmployeeCount(ds.getDepartment().getId());
			departmentWeekTotal = ds.getBasescore() * (float)departmentWeekEmployee;
			departmentWeekWaitting = scoreBean.getDepartmentWaittingScores(ds.getDepartment().getId(),cal.getTime(),cal2.getTime());
			departmentWeekReject  = scoreBean.getDepartmentRejectedScores(ds.getDepartment().getId(),cal.getTime(),cal2.getTime());
			departmentWeekNotSubmit = scoreBean.getDepartmentNotSubmitScores(ds.getDepartment().getId(), cal.getTime(), cal2.getTime());
