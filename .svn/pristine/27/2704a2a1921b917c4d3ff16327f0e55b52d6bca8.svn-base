	...
	<style type="text/css">
		...
		<!-- Side bar ****************************************************************** -->
				...
				<table>
					...
					<c:forEach items="${actionBean.mappers}" var="mapper" varStatus="loop">
						<c:choose>
							<c:when test="${mapper.vehicle.status == statusE}">
								<tr class="expired">
							</c:when>
							<c:otherwise>
								<tr class="alt">
							...
							<td>
								<stripes:checkbox name="selectedVehicles" value="${mapper.vehicle.id}"/>
							</td>
							<td>${mapper.vehicle.selfid}</td>
							<td><a href="${pageContext.request.contextPath}/actionbean/VehicleProfile.action?targetId=${mapper.vehicle.id}&vehicleDetail=">${mapper.vehicle.vid}</a></td>
							<td>${mapper.vehicle.recordid}</td>
							<td><a href="${pageContext.request.contextPath}/actionbean/VehicleTeam.action?teamId=${mapper.vehicle.team.team.id}">${mapper.vehicle.team.team.name}</a></td>
							<td>${mapper.vehicle.subcompany}</td>
							<td>${mapper.vehicle.throwdateStr}</td>
							<td>${mapper.vehicle.dateinvalidateStr}</td>
							<td>${mapper.vehicle.totalmiles}</td>
						</tr>
