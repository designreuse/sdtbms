...
*+html .top_rightbtn {
	...
	margin-top: 10px;
		...
		<script type="text/javascript"
			src="${pageContext.request.contextPath}/js/hr.js"></script>
		<script type="text/javascript">
			function addRow() {
	...
	vertical-align: middle;
		...
		<div id="sub-nav">
			<div class="page-title">
				<h1>人事管理</h1>
				<span><a href="#" title="Layout Options">人事</a> > <a href="#"
					title="Two column layout">档案管理</a> > 查看</span>
			</div>
		</div>
		...
		<div id="page-layout">
			<div id="page-content">
				<div id="page-content-wrapper">
					...
					<div id="rightcontent">
						<div class="topbtntile">
							<!-- 新建档案  Dialog-->
							<div id="hr_top_button_bar" class="top_leftbtn">
								&nbsp; <a class="btn ui-state-default ui-corner-all"
									href="javascript:location.reload();"> 刷新 </a>
								<ss:secure roles="employee_data_download">
									<a id="btn_download_employees"
										class="btn ui-state-default ui-corner-all"
										href="javascript:void;"> 导出员工花名册<input type="hidden"
										value="${pageContext.request.contextPath}/actionbean/Filedownload.action" />
									</a>
								</ss:secure>

								<ss:secure roles="employee_driver_data_download">
									<a id="btn_download_drivers"
										class="btn ui-state-default ui-corner-all"
										href="javascript:void;"> 导出驾驶员资料<input type="hidden"
										value="${pageContext.request.contextPath}/actionbean/Filedownload.action" />
									</a>
								</ss:secure>

								<ss:secure roles="employee_coor_data_download">
									<a id="btn_download_month_report"
										class="btn ui-state-default ui-corner-all"
										href="javascript:void;"> 导出月报表<input type="hidden"
										value="${pageContext.request.contextPath}/actionbean/Filedownload.action" />
									</a>
									<div id="btn_download_report_dialog" title="导出报表">
										<stripes:form id="form_get_coordination"
											beanclass="com.bus.stripes.actionbean.FiledownloadActionBean">
											<table>
												<tr>
													<td>类型:</td>
												...
												<tr>
													<td>起始日期:</td>
													<td><stripes:text name="startdate"
															formatPattern="yyyy-MM-dd"
															class="datepickerClass required" /></td>
													<td>截止日期:</td>
													<td><stripes:text name="enddate"
															formatPattern="yyyy-MM-dd"
															class="datepickerClass required" /></td>
							...
							<ss:secure roles="employee_edit">
								<div class="top_rightbtn">
									<a id="btn_new_document_link"
										class="btn ui-state-default ui-corner-all" href="#"> <span
										class="ui-icon ui-icon-person"></span> 新建档案
									</a>
								</div>
							</ss:secure>
						</div>
			 <ss:secure roles="employee_edit">
				...
				<stripes:form id="form_new_employee" beanclass="com.bus.stripes.actionbean.EmployeeActionBean" focus="">
					...
					<div>
						...
						<ss:secure roles="employee_add_contract">
							<div id="btn_new_contract_dialog" title="新建档案">
								<stripes:form id="form_new_contract"
									beanclass="com.bus.stripes.actionbean.EmployeeActionBean"
									focus="">
									<stripes:errors />
									<table>
										<tr>
											<td colspan=4 style="text-align: center;"><label
												id="contract_label_name"></label></td>
										</tr>
										<tr>
											<td>合同类型:</td>
											<td><stripes:select name="contract.type">
													<stripes:option value="">请选择....</stripes:option>
													<stripes:options-collection
														collection="${actionBean.contracttype}" label="label"
														value="value" />
												</stripes:select></td>
											<td>生效日期:</td>
											<td><stripes:text name="contract.activedate"
													formatPattern="yyyy-MM-dd" class="datepickerClass" /><input
												id="contract_target_id" type="hidden" name="targetId"
												value="" /></td>
										</tr>
										<tr>
											<td>开始日期:</td>
											<td><stripes:text name="contract.startdate"
													formatPattern="yyyy-MM-dd" class="datepickerClass" /></td>
											<td>结束日期:</td>
											<td><stripes:text name="contract.enddate"
													formatPattern="yyyy-MM-dd" class="datepickerClass" /></td>

										</tr>
										<tr>
											<td>附录:</td>
											<td colspan=3><stripes:textarea name="contract.remark" /></td>
										</tr>
									</table>
								</stripes:form>
							</div>
					...
					<div>
						文件上传
						<stripes:form id="file_submit_form"
							beanclass="com.bus.stripes.actionbean.EmployeeActionBean">
							<ss:secure roles="employee_datafile_upload">
								<Label>员工资料平台 :</Label>
								<stripes:file name="employeeexcel" id="file_employee" />
								<stripes:submit name="employeefileupload" value="在职" />
								<stripes:submit name="employeeresignfileupload" value="辞职" />
								<br />
							</ss:secure>
							<ss:secure roles="employee_idcheck_file_upload">
								<Label>检查员工ID:</Label>
								<stripes:file name="checkIds" id="file_employee_check" />
								<stripes:submit name="checkids" value="提交" />
								<br />
							</ss:secure>
							<ss:secure roles="employee_datafile_upload">
								<Label>驾驶员文件上传:</Label>
								<stripes:file name="drivers" id="file_drivers" />
								<stripes:submit name="driverlisences" value="提交" />
								<br />
							</ss:secure>

							<ss:secure roles="employee_coordination_file_upload">
								<Label>调动文件csv:</Label>
								<stripes:file name="coordinatorfile" id="file_coordinator" />
								<stripes:submit name="coordinatorfileupload" value="提交" />
								<br />
							</ss:secure>
							...
							<ss:secure roles="administrator_system">
								<Label>导入文件:</Label>
								<stripes:file name="processfile" />
								<stripes:submit name="processfileupload" value="驾驶员分车队csv" />
								<br />
							</ss:secure>
					...
					<!--  Display Employee   -->
					<div>
						<div class="inner-page-title">员工档案详细信息</div>
						<div class="hastable">

							<table>
								<thead>
									<stripes:form id="employee_selection_form"
										beanclass="com.bus.stripes.actionbean.EmployeeActionBean"
										focus="">
										<tr>
											<td colspan=13 style="text-align: left"><stripes:submit
													name="prevpage" value="上页" /> 页码: <stripes:text
													name="pagenum" />/<stripes:label
													name="${actionBean.totalcount}" /> <stripes:submit
													name="nextpage" value="下页" /> 显示数量:<stripes:text
													name="lotsize" /> <label>总数:${actionBean.recordsTotal}
													行</label></td>
										</tr>
										<tr>
											<td colspan=11 style="text-align: left"><Label
												class='selector'>名称:</Label>
											<stripes:text name="employeeselector.name" /> <Label
												class='selector'>工号:</Label>
											<stripes:text name="employeeselector.workerid" /> <Label
												class='selector'>入职期从:</Label>
											<stripes:text name="employeeselector.workstartdate"
													formatPattern="yyyy-MM-dd" class="datepickerClass" /> 到<stripes:text
													name="employeeselector.workenddate"
													formatPattern="yyyy-MM-dd" class="datepickerClass" /></td>
										...
										<tr>
											<td colspan=11 style="text-align: left"><Label
												class='selector'>性别:</Label>
											<stripes:radio value="" name="employeeselector.sex" />不限<stripes:radio
													value="男" name="employeeselector.sex" />男<stripes:radio
													value="女" name="employeeselector.sex" />女 <br /> <Label
												class='selector'>职称:</Label>
											<stripes:select name="employeeselector.workertype">
													<stripes:option value="">不限</stripes:option>
													<stripes:options-collection
														collection="${actionBean.workertype}" label="label"
														value="value" />
												</stripes:select> <br /> <Label class='selector'>籍贯:</Label>
											<stripes:text name="employeeselector.pob" /> <Label
												class='selector'>驾驶员:</Label>
											<stripes:text name="employeeselector.driver" /> <Label
												class='selector'>所属镇街:</Label>
											<stripes:select name="employeeselector.placebelong">
													<stripes:option value=""></stripes:option>
													<stripes:options-collection
														collection="${actionBean.placebelongs}" label="label"
														value="label" />
												</stripes:select> <Label class='selector'>所属车队:</Label>
											<stripes:select name="employeeselector.teamid">
													<stripes:option value=""></stripes:option>
													<stripes:options-collection
														collection="${actionBean.driverteams}" label="name"
														value="id" />
												</stripes:select> <%-- 								年龄:<stripes:text name="employeeselector.age"/> --%>
											</td>
											<td rowspan=3><stripes:select name="employeeselector.ds"
													multiple="multiple" style="height:200px;">
													<stripes:option value="">不限</stripes:option>
													<stripes:options-collection
														collection="${actionBean.department}" label="label"
														value="value" />
												</stripes:select></td>
											<td rowspan=3><stripes:select name="employeeselector.ps"
													multiple="multiple" style="height:200px;">
													<stripes:option value="">不限</stripes:option>
													<stripes:options-collection
														collection="${actionBean.position}" label="label"
														value="value" />
												</stripes:select></td>
										</tr>
										<tr>
											<td colspan=11 style="text-align: left"><Label
												class='selector'>出生月份:</Label> <stripes:select
													name="employeeselector.birthmonth">
													<stripes:option value="">不限</stripes:option>
												...
												</stripes:select> <Label class='selector'>职级:</Label>
											<stripes:select name="employeeselector.joblevel">
													<stripes:option value="">不限</stripes:option>
													<stripes:options-collection
														collection="${actionBean.joblevel}" label="label"
														value="value" />
												</stripes:select> <Label class='selector'>学历:</Label>
											<stripes:select name="employeeselector.qualification">
													<stripes:option value="">请选择....</stripes:option>
													<stripes:options-collection
														collection="${actionBean.qualification}" label="label"
														value="value" />
												</stripes:select> <br /> <Label class='selector'>民族:</Label>
											<stripes:select name="employeeselector.ethnic">
													<stripes:option value="">不限</stripes:option>
													<stripes:options-collection
														collection="${actionBean.ethnic}" label="label"
														value="value" />
												</stripes:select> <Label class='selector'>政治面貌:</Label>
											<stripes:select name="employeeselector.politicalstatus">
													<stripes:option value="">不限</stripes:option>
													<stripes:options-collection
														collection="${actionBean.politicalStatus}" label="label"
														value="value" />
												</stripes:select> <Label class='selector'>婚姻:</Label>
											<stripes:select name="employeeselector.marriage">
													<stripes:option value="">不限</stripes:option>
													<stripes:options-collection
														collection="${actionBean.marriage}" label="label"
														value="value" />
												</stripes:select> <Label class='selector'>户籍:</Label>
											<stripes:select name="employeeselector.domiciletype">
													<stripes:option value="">不限</stripes:option>
													<stripes:options-collection
														collection="${actionBean.domiciletypes}" label="label"
														value="value" />
												</stripes:select> <Label class='selector'>健康状况:</Label>
											<stripes:select name="employeeselector.healthstatus">
													<stripes:option value="">不限</stripes:option>
													<stripes:options-collection
														collection="${actionBean.healthstatus}" label="label"
														value="value" />
												</stripes:select></td>
										</tr>
										<tr>
											<td colspan=11 style="text-align: left"><Label
												class='selector'>特殊身份(例:军人):</Label>
											<stripes:text name="employeeselector.army" /> <br /> <Label
												class='selector'>日期:</Label>
											<stripes:radio name="employeeselector.date" value="0" />不限 <stripes:radio
													name="employeeselector.date" value="1" />入职时间<stripes:radio
													name="employeeselector.date" value="2" />按年龄排 <br /> <Label
												class='selector'>员工状态:</Label>
											<stripes:radio name="employeeselector.status" value="A" />在职<stripes:radio
													name="employeeselector.status" value="E" />辞职<stripes:radio
													name="employeeselector.status" value="A,E" />全部 <br /> <stripes:submit
													name="filter" value="提交" /> <ss:secure
													roles="employee_data_download">
					...
					<ss:secure roles="employee_resign">
						<div id="btn_resign_date_dialog" title="辞职日期">
							<stripes:form
								beanclass="com.bus.stripes.actionbean.EmployeeActionBean">
						辞职日期:<stripes:text name="resigndate" formatPattern="yyyy-MM-dd"
									class="datepickerClass" />
								<input id="resignemployeeId" type="hidden" name="targetId"
									value="" />
								<br />
					...
					<ss:secure roles="employee_edit">
						<div id="btn_rejoin_date_dialog" title="复职日期">
							<stripes:form
								beanclass="com.bus.stripes.actionbean.EmployeeActionBean">
						复职日期:<stripes:text name="resigndate" formatPattern="yyyy-MM-dd"
									class="datepickerClass" />
								<input id="rejoinemployeeId" type="hidden" name="targetId"
									value="" />
								<br />
